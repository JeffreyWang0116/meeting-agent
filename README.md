...
> 轉錄與分析用**不同的模型**：轉錄吃掉絕大多數 API 請求（即時聆聽每 45 秒一次）但不需要聰明模型，走 `TRANSCRIBE_MODEL`（預設 `gemini-flash-lite-latest`，免費額度高）；分析（萃取代辦、決議）每場會議只呼叫 1~3 次，走 `GEMINI_MODEL` 維持品質。撞到 429 就是配額用完——免費層 `gemini-3.5-flash` 每把 key 每天僅 20 次，額度可到 [AI Studio](https://aistudio.google.com/rate-limit) 查詢。
...
4. 重新部署。啟動後 `GET /api/config` 的 `store_backend` 會顯示 `firestore` 代表已生效
