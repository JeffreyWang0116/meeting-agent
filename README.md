...
- **講者姓名**：詞彙表中標成「人名」的項目會在辨識講者時統一姓名寫法，同一個人每場都叫同一個名字（名冊已併入詞彙表，只需維護一份）
- **預錄聲音辨識人**（即時聆聽，選用）：開始聆聽前請每位與會者各錄約 10 秒並標上姓名，結束分析時用聲紋比對把逐字稿的「講者A/B/C」換成真實姓名。**沒勾選就完全不啟用**——不錄音、不上傳、不多打任何 API；樣本只存在該場 session，結束即刪
...
| 聲紋接力最多記幾位講者 | `VOICE_RELAY_MAX_SPEAKERS` | **目前停用**（程式碼保留） | 見下方「分段時講者跨段一致」的說明 |
...
>
> **分段時講者跨段一致（聲紋接力，目前停用但程式碼保留）**：分段轉錄每段都是模型的全新呼叫，它沒有跨段記憶，純文字提示「請沿用同一個代號指稱同一個人」時模型其實從沒聽過那個人的聲音，等於要它憑空判斷——實務上常見同一位委員在不同段被標成不同代號。已實作的做法是每段轉完，替新出現的講者代號（挑該段內容最長的一句）剪一小截嗓音存進「聲音簿」，從下一段開始當參考音訊一起送給模型。**目前沒有接上去**（`app/main.py` 裡那行是註解），原因是每段轉錄都要重傳全部樣本、重試也會重傳，一支 77 分鐘的檔約多 240~400 次上傳往返、多花數分鐘，而「模型是否真的因此標對代號」還沒用真檔驗證過。取消 `main.py` 那行註解即可啟用。現行行為仍是純文字提示＋分段重疊（`TRANSCRIBE_OVERLAP_SECONDS`）接力。
...
- **講者名冊不注入轉錄 prompt**：名冊只給 SpeakerNamerAgent。轉錄階段刻意一律輸出「講者A/B/C」代號——模型沒有跨段記憶，給它一份姓名清單，同一個人會在聽得到稱謂的段落標姓名、聽不到的段落標代號，整份逐字稿混雜兩種標籤。姓名統一在最後一步補回，那時整份逐字稿都在手上、判斷只做一次。名冊在該 prompt 裡的定位也寫死成「僅供姓名寫法參考，不是判斷依據」，否則模型會把名單當候選答案，硬把每個代號都塞給名單上的人
- **聲紋比對是獨立一步，不塞進轉錄**：把聲音樣本餵進轉錄、讓模型直接標姓名，會正面撞上「轉錄一律輸出代號」那條規則——`SPEAKER_RE` 只認代號格式，模型一吐姓名，`speaker_label_ratio` 就歸零並觸發無謂的重試，跨段講者一致性也跟著失效。而且即時聆聽每 45 秒轉錄一次，每次都重帶樣本，一小時會議多出約 10 萬 input token。改成結束時**單獨打一次**比對呼叫：只送註冊樣本＋幾段代表性的會議錄音（每個代號挑它講最多話的那一段，見 `pick_evidence_chunks`），音訊量與會議長度脫鉤，一場約 220 秒
- **聲紋結果勝過文字線索，但一樣要過驗證**：`SpeakerNamerAgent` 靠「請王委員發言」這類文字線索推斷，聲紋則是使用者親自錄的第一手資訊，兩者衝突時聲紋贏。但比對結果只是「建議」——姓名仍須通過 `is_safe_name`、且改寫一律走 `apply_speaker_names`（驗行數、時間標記、重複姓名）。模型回傳沒註冊過的名字（從逐字稿內容猜的）會被直接丟掉：聲紋比對的依據只能是嗓音
- **聲音樣本不落地保存**：聲紋是生物特徵資料。樣本只存在該場 session 的暫存目錄，跟著錄音段在 `finish()` 一起刪除，也受閒置 TTL 回收——不進資料庫，省掉一整類隱私與跨帳號外洩的問題
...
│   ├── speaker_namer_agent.py # 講者代號→姓名（選用）：文字線索判斷，本地套用並驗證
