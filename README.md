...
- **專門的講者分離**（選用，設 `PYANNOTE_API_KEY`）：用 pyannoteAI 聲學模型對整份音檔分群，再依時間戳重標 Gemini 逐字稿的講者——77 分鐘協商實測從 3 位分到 19 位；預錄聲音辨識人改用 voiceprint 比對。沒設就照舊，失敗也自動退回（見「專門的講者分離」）
...
| 聲紋比對門檻（0~100） | `VOICEPRINT_MATCH_THRESHOLD` | `50` | 實測本人 89、非本人 ≤28 |
...
- **預錄聲音辨識人**：結束時改用 pyannote voiceprint 比對姓名（聆聽中第 2 段的提早回報照舊走 Gemini）。門檻 `VOICEPRINT_MATCH_THRESHOLD=50`：實測本人 89 分、不是本人 16~28 分；API 預設 0 時，片段裡根本沒出現的人被硬配給一位只講 4 秒的人
- 聲紋接力在這個模式下自動跳過（代號反正會被整份重標蓋掉）
- **任何失敗都退回 Gemini 的代號**：pyannote 當掉、額度用完、逾時、ffmpeg 壓不動，轉錄與分析照常完成
- 本地 Whisper 引擎不啟用：它的輸出沒有時間戳，對不回任何一行

**費用**：新帳號 30 天試用含 150 小時與 10 個 voiceprint（需公司或學校信箱註冊，Gmail 不行）；之後要訂閱（Developer 方案 €19/月起），沒訂閱 API 會回 402，系統就自動退回 Gemini 標註。voiceprint 按**建立次數**計費：預錄 4 個人＝用掉 4 個。
