...
| 預錄聲音辨識人改用 voiceprint | `PYANNOTE_VOICEPRINT_ENABLED` | `0`（關） | 每預錄一人扣 1 個 voiceprint（試用僅 10 個）；關閉時有預錄的場次整場走 Gemini |
...
- **預錄聲音辨識人**：**預設不使用 voiceprint**（`PYANNOTE_VOICEPRINT_ENABLED=0`）——有預錄樣本的即時聆聽整場照舊走 Gemini 標講者、比對姓名，不消耗 voiceprint。原因是 pyannote 純分群給不出姓名，而 Gemini 認出的名字掛在它自己的代號上，重標後對不上，名字會整組消失。設成 1 才會在結束時改用 pyannote voiceprint 比對（聆聽中第 2 段的提早回報照舊走 Gemini）。門檻 `VOICEPRINT_MATCH_THRESHOLD=50`：實測本人 89 分、不是本人 16~28 分；API 預設 0 時，片段裡根本沒出現的人被硬配給一位只講 4 秒的人
- 聲紋接力在這個模式下自動跳過（代號反正會被整份重標蓋掉）
- **任何失敗都退回 Gemini 的代號**：pyannote 當掉、額度用完、逾時、ffmpeg 壓不動，轉錄與分析照常完成
- 本地 Whisper 引擎不啟用：它的輸出沒有時間戳，對不回任何一行

**費用**：新帳號 30 天試用含 150 小時與 10 個 voiceprint（需公司或學校信箱註冊，Gmail 不行）；之後要訂閱（Developer 方案 €19/月起），沒訂閱 API 會回 402，系統就自動退回 Gemini 標註。voiceprint 按**建立次數**計費：開啟時預錄 4 個人＝用掉 4 個，所以預設關閉。
...
- **聲音樣本不落地保存**：聲紋是生物特徵資料。樣本只存在該場 session 的暫存目錄，跟著錄音段在 `finish()` 一起刪除，也受閒置 TTL 回收——不進資料庫，省掉一整類隱私與跨帳號外洩的問題。啟用 pyannote 時樣本與錄音會上傳到 pyannoteAI 暫存區處理（24~48 小時內自動刪除），voiceprint 也只在該次比對中使用、不另外保存
...
tests/                    # pytest 測試（836，全部離線、不需金鑰；前端講者判斷另需 node）
