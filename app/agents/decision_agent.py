...
# 同理不放「決議」「摘要」，那些通用欄位已經有了。
#
# 「一般會議」與「其它」刻意留空：它們是收容所，內容什麼形狀都有可能，
# 硬塞固定欄位只會逼模型對著沒談到的主題生出空區塊，或更糟——編一個來填滿。
KIND_SECTIONS = {
    "銷售拜訪": {
    ...
    "一對一": {
        "目標與進度": "當事人談到的目標、負責的事情目前到哪裡",
        "遇到的困難": "卡住的地方、需要的資源或支援，用當事人自己的說法",
        "收到的回饋": "會中實際說出口的回饋，雙向都算；禁止補上沒說過的評語或推測語氣",
        "職涯與發展": "想學的技能、想走的方向、對角色或調動的想法；沒聊到就留空陣列",
    },
    "面試": {
        "技術能力": "候選人展現的技術深度，每條都附上對話中的具體事例，不要只寫結論",
        "相關經驗": "做過的專案與擔任的角色，包含規模、時間與實際負責的部分",
        "溝通表達": "回答的清晰度與互動方式，同樣要有對話佐證；禁止寫個性評價",
        "動機與期望": "為什麼想來、對職務的期待、時間與待遇上提到的條件",
    },
    "需求訪談": {
        "現況做法": "受訪者目前實際怎麼做這件事，含用到的工具與流程",
        "痛點": "現況哪裡讓對方難受，盡量保留對方原本的描述",
        "明確需求": "對方主動講出來、要求要有的東西；你推論出來的放 pending_items",
        "限制條件": "預算、時程、法規、既有系統或人力上的限制",
    },
    "專案會議": {
        "進度與里程碑": "各項工作的實際狀態，以及與原訂排程的落差",
        "阻礙與風險": "擋住進度的問題與預見的風險，有提到因應方式就一併寫上",
    },
    "語音備忘錄": {
        "想法與靈感": "講者提到但還沒成形的念頭、觀察與點子；能動手做的請放 todos",
    },
}
